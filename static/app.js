(function () {
  const el = {
    ...
    poolStatusNote: document.getElementById('poolStatusNote'),
  };
  ...
  function renderImportReport(report) {
    ...
    const detailList = (items, emptyText, formatter) => {
      if (!items || !items.length) return `<p class="report-empty">${emptyText}</p>`;
      return `<ul class="report-details">${items.map(formatter).join('')}</ul>`;
    };
    ...
    el.importReport.innerHTML = `
      <div class="report-header">
        <div>
          <div class="report-kicker">IMPORT REPORT</div>
          <strong>${report.rows_found} data row${report.rows_found === 1 ? '' : 's'} checked</strong>
        </div>
        <span class="report-success">${report.imported} imported</span>
      </div>
      <div class="report-grid">
        <div class="report-stat good"><b>${report.imported}</b><span>Imported</span></div>
        <div class="report-stat neutral"><b>${report.duplicates}</b><span>Duplicates removed</span></div>
        <div class="report-stat merge"><b>${report.merged}</b><span>Names merged</span></div>
        <div class="report-stat bad"><b>${report.rejected}</b><span>Rejected</span></div>
      </div>
      <div class="report-columns">
        <details ${report.merged ? 'open' : ''}>
          <summary>Merged names (${report.merged})</summary>
          ${detailList(report.merged_detail, 'No names needed merging.', item => `<li><span>Row ${item.row}</span><b>${escapeHtml(item.raw)}</b> <span class="arrow">→</span> <b>${escapeHtml(item.canonical)}</b><small>${escapeHtml(item.reason)}</small></li>`)}
        </details>
        <details ${report.duplicates ? 'open' : ''}>
          <summary>Duplicates removed (${report.duplicates})</summary>
          ${detailList(report.duplicates_detail, 'No duplicate rows found.', item => `<li><span>Row ${item.row}</span><b>${escapeHtml(item.name)}</b><span>${rupee(item.amount)}</span></li>`)}
        </details>
        <details ${report.rejected ? 'open' : ''}>
          <summary>Rejected rows (${report.rejected})</summary>
          ${detailList(report.rejected_detail, 'No invalid rows found.', item => `<li><span>Row ${item.row}</span><b>${escapeHtml(item.name || 'Unknown')}</b><span>${escapeHtml(item.reason)}</span></li>`)}
        </details>
      </div>
    `;
  }
  ...
  el.csvFile.addEventListener('change', () => {
    ...
    el.fileName.textContent = file ? file.name : 'Choose CSV file';
    el.importBtn.disabled = !file;
  });
  ...
  el.importBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      await refresh();
      el.csvFile.value = '';
      el.fileName.textContent = 'Choose CSV file';
    } catch (e) {
      alert(e.message);
    } finally {
      el.importBtn.disabled = !el.csvFile.files[0];
      el.importBtn.textContent = 'Import contributions';
    }
  });
  ...
  function render(state) {
    ...
    if (status.remaining > 0.01) {
      el.poolStatusNote.textContent = `${rupee(status.remaining)} still needed`;
    } else if (status.surplus > 0.01) {
      el.poolStatusNote.textContent = `${rupee(status.surplus)} above target`;
    } else {
      el.poolStatusNote.textContent = 'target fully collected';
    }
