...
- **Data import:** deterministic CSV cleaning in `importer.py` (normalization, validation, duplicate detection, conservative name matching)
...
├── importer.py                 # messy CSV cleaning + import report
├── templates/
│   └── index.html             # page structure
├── static/
│   ├── style.css               # passbook/ledger styling
│   ├── app.js                  # fetches the API, renders the passbook
│   └── demo_contributions.csv  # intentionally messy demo file for the build round
├── tests/
...
**Messy CSV import:** upload a CSV with `name`, `amount`, and optional `note`. The importer accepts common rupee formats, rejects invalid rows, detects exact duplicate contribution rows, and conservatively merges obvious name variants. The import report shows rows checked, imported, duplicates removed, names merged, and rejected rows. Valid imported contributions are inserted into the same payments table used by manual entries, so the existing balance and settlement logic remains the single source of truth. A deliberately messy demo file is included at `static/demo_contributions.csv`.
...
| POST | `/api/import` | multipart/form-data `file` | Clean and import a CSV contribution list; returns an audit report |
