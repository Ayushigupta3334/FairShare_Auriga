...
def import_contributions():
    """Import a messy CSV, clean it, merge names, and insert valid payments."""
    ...
    try:
        ...
        return jsonify({"error": "The CSV must be UTF-8 encoded."}), 400

    conn = db.get_connection()
    try:
        existing_rows = conn.execute("SELECT id, name FROM members ORDER BY id").fetchall()
        existing_names = [r["name"] for r in existing_rows]

        report = parse_csv(text, existing_names)
        if report["rejected"] and report["imported"] == 0 and report["rows_found"] > 0:
            # Still return a useful report; the organiser can see exactly why
            # nothing was accepted instead of receiving a generic server error.
            return jsonify({"ok": True, "report": report})

        # Resolve each cleaned name to an existing member or create one.
        member_ids = {r["name"].casefold(): r["id"] for r in existing_rows}
        for item in report["import_rows"]:
            key = item["name"].casefold()
            member_id = member_ids.get(key)
            if member_id is None:
                cur = conn.execute("INSERT INTO members (name) VALUES (?)", (item["name"],))
                member_id = cur.lastrowid
                member_ids[key] = member_id
            conn.execute(
                "INSERT INTO payments (member_id, amount, note) VALUES (?, ?, ?)",
                (member_id, item["amount"], item["note"]),
            )
        conn.commit()
    except Exception:
        conn.rollback()
